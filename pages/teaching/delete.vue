...
const updateSecondOptions = async () => {
	...
	model.courseSelected =
		state.secondOptions.length !== 0 ? state.secondOptions[0].value : undefined
}
...
<template>
	<div class="flex flex-col gap-3">
		...
		<ZodForm @submit="submit">
			<ZodKit
				v-model="model.teacherSelected"
				label="Teacher"
				type="select"
				name="teacherId"
				:options="teachersOptions"
				@change="
					changeValueFirstOption(model.teacherSelected), updateSecondOptions()
				"
			/>
			<ZodKit
				v-model="model.courseSelected"
				label="Course"
				type="select"
				name="course"
